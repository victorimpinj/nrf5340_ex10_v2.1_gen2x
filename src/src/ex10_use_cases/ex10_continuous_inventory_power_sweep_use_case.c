...
#include "ex10_use_cases/ex10_continuous_inventory_power_sweep_use_case.h"


struct PowerSweepUseCaseConfigs
{
    int16_t const* power_levels_cdbm;
...
/**
 * Ex10ContinuousInventoryPowerSweep private state variables.
...
static struct PowerSweepUseCaseConfigs psucc = {
    .power_levels_cdbm = NULL,
    .num_power_levels  = 0,
    .iterator          = 0,
};
...
static struct Ex10Result get_current_power_sweep_level(int16_t* current_power)
{
    if (psucc.power_levels_cdbm == NULL)
    {
        return make_ex10_sdk_error(Ex10ModuleUseCase, Ex10SdkErrorNullPointer);
    }
    *current_power = psucc.power_levels_cdbm[psucc.iterator];
    return make_ex10_success();
}

static void update_current_power_sweep_level(void)
{
    psucc.iterator++;
    if (psucc.iterator >= psucc.num_power_levels)
    {
        psucc.iterator = 0;
    }
}

static void set_power_sweep_levels(int16_t const* powers_cdbm,
                                   uint32_t       num_powers)
{
    psucc.power_levels_cdbm = powers_cdbm;
    psucc.num_power_levels  = num_powers;
    psucc.iterator          = 0;
}
...
static struct Ex10Result round_done_update_power(int16_t curr_power_cdbm,
                                                 int16_t new_power_cdbm)
{
    ...
    uint32_t frequency_khz                = region->get_active_channel_khz();

    bool const        temp_comp_enabled = true;
    uint16_t          temperature_adc   = 0;
    struct Ex10Result ex10_result =
        get_ex10_rf_power()->measure_and_read_adc_temperature(&temperature_adc);
    if (ex10_result.error == false)
    {
        struct PowerConfigs curr_power_config =
        ...
        ex10_result =
            get_ex10_dynamic_power_ramp()->update_power(&curr_power_config,
                                                        &agg_buffer,
                                                        new_power_cdbm,
                                                        frequency_khz,
                                                        temperature_adc);
...
static bool interrupt_handler(struct InterruptStatusFields irq_status)
{
    ...
    else
    {
        ...
        bool attempt_dynamic_power_ramp = false;
        switch (inventory_state.done_reason)
        {
            case InventorySummaryDone:
                attempt_dynamic_power_ramp = true;
        ...
        {
            // now that we know we are going on, we update the power if
            // needed
            if (attempt_dynamic_power_ramp)
            {
                int16_t const curr_power_cdbm = inventory_params.tx_power_cdbm;
                // Since the round is done, update to use the new power
                // level in the power sweep sequence.
                update_current_power_sweep_level();
                ex10_result = get_current_power_sweep_level(
                    &inventory_params.tx_power_cdbm);
                if (ex10_result.error)
                {
                    handle_continuous_inventory_error(ex10_result, time_us);
                ...
                // If ramped up, use power sweep update. Otherwise the next
                // ramp will use the next power
                if (get_ex10_rf_power()->get_cw_is_on())
                {
                    int16_t const new_power_cdbm =
                        inventory_params.tx_power_cdbm;
                    ex10_result = round_done_update_power(curr_power_cdbm,
                                                          new_power_cdbm);
                    if (ex10_result.error)
                    {
                        handle_continuous_inventory_error(ex10_result, time_us);
...
static void fifo_data_handler(struct FifoBufferNode* fifo_buffer_node)
{
    ...
    while (bytes.length > 0u)
    {
        ...
        {
            // The Continuous Inventory Summary packet was created before
            // counting the total number of tags read, so update that field now
    ...
    struct Ex10PowerSweepUseCaseParameters* params)
{
    ...
    inventory_params.inventory_config.use_tag_read_extended = 0;
    ...
    // Update inventory_params.tx_power_cdbm with the current level to use
    struct Ex10Result ex10_result =
        get_current_power_sweep_level(&inventory_params.tx_power_cdbm);
    if (ex10_result.error)
    {
        return ex10_result;
    }
    ...
    if (inventory_params.inventory_config.tag_focus_enable)
    {
        if (params->dual_target == true)
        {
            ex10_printf(
                ...
                "running the use case in a single target configuration.\n");
        }
    }
...
// clang-format off
static struct Ex10ContinuousInventoryPowerSweepUseCase ex10_continuous_inventory_power_sweep_use_case = {
    .init                                 = init,
    .deinit                               = deinit,
    ...
    .set_power_sweep_levels               = set_power_sweep_levels,
};
// clang-format on
...
{
    return &ex10_continuous_inventory_power_sweep_use_case;
}
